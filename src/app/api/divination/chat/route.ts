...
function getDivinationNumbers(): [number, number, number] {
  return [
...
function calculateHexagram(num1: number, num2: number, num3: number): DivinationResult {
  const upperTrigram = TRIGRAMS[num2 % 8] ?? TRIGRAMS[1];
  const lowerTrigram = TRIGRAMS[num1 % 8] ?? TRIGRAMS[1];
  const hexagramKey = `${upperTrigram.name}${lowerTrigram.name}`;

  let hexagram: Hexagram;
  if (hexagramKey in HEXAGRAMS) {
    hexagram = HEXAGRAMS[hexagramKey];
  } else {
    hexagram = {
      num: 0,
      name: `${upperTrigram.name}${lowerTrigram.name}卦`,
      meaning: `上${upperTrigram.element}下${lowerTrigram.element}之象。`,
      fortune: '中平',
    };
  }

  const changingLine = (num3 % 6) === 0 ? 6 : (num3 % 6);
...
function determineIntent(question: string): 'DIVINATION' | 'PERSONA' {
  ...
  const divinationKeywords = ['占卜', '算命', '運勢', '吉凶', '未來', '發展', '如何', '是否', '會不會'];
  const personaKeywords = ['陳老師', '你', '您', '介紹', '背景', '聯絡'];

  const divinationScore = divinationKeywords.filter((kw) => questionLower.includes(kw)).length;
  const personaScore = personaKeywords.filter((kw) => questionLower.includes(kw)).length;

  if (divinationScore > personaScore) return 'DIVINATION';
  if (personaScore > 0) return 'PERSONA';
  return 'DIVINATION';
}
...
async function getAiResponse(question: string, systemPrompt: string): Promise<string> {
  ...
  try {
    ...
    return response.choices[0].message.content?.trim() ?? '抱歉，我目前無法回答這個問題。';
  } catch {
...
export async function POST(request: NextRequest) {
  let body: { message?: string; numbers?: unknown };
  try {
  ...
  const userNumbers = body.numbers;
  ...
  if (intent === 'DIVINATION') {
    ...
    if (
      Array.isArray(userNumbers) &&
      userNumbers.length === 3 &&
      userNumbers.every((n) => typeof n === 'number')
    ) {
      [num1, num2, num3] = userNumbers as [number, number, number];
    } else {
      [num1, num2, num3] = getDivinationNumbers();
    }
